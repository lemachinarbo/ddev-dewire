...
choose_setup_mode() {
  echo
  log_info "Which environments do you want to configure?:"
  log_option 1 "Local only        – for development"
  log_option 2 "Deployment only   – staging/production/etc…"
  log_option 3 "Both              – local + deployment"
  local selection
  while true; do
    echo
    log_ask "Choose an option [3]: "
    read -r selection
    case "${selection:-3}" in
    1)
      SETUP_MODE="local"
